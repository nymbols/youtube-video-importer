...
;(function($){
	$(document).ready(function(){
		...
		var actions = yti_bulk_actions.actions,
		...
		var messageTop = $('<span></span>', {'class':'yti-message'}).insertAfter( $('#doaction') ),
			messageBottom = $('<span></span>', {'class':'yti-message'}).insertAfter( $('#doaction2') );
		...
		$('#posts-filter').submit(function(e){
			...
			if( is_loading ){
				e.preventDefault();
				$(messageTop).html( yti_bulk_actions.wait_longer );
				$(messageBottom).html( yti_bulk_actions.wait_longer );
				return;
			}
			
			if( 'yti_thumbnail' == val_top || 'yti_thumbnail' == val_bottom ){
				e.preventDefault();
				...
				$(messageTop).html( yti_bulk_actions.wait );
				$(messageBottom).html( yti_bulk_actions.wait );
				
				if( 'yti_thumbnail' != val_top ){
					$(select_top).val('yti_thumbnail');
				}
				...
				$.ajax({
					...
					success	: function( response ){
						...
						if( '' == response || !response.data ){
							$(messageTop).html( yti_bulk_actions.maybe_error );
							$(messageBottom).html( yti_bulk_actions.maybe_error );
							return;
						}
