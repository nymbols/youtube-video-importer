 ...
 * to user settings.
 */
abstract class YTI_Video_Post_Type{
	 ...
	 * @var string
	 */
	protected $playlist_type				= 'yti_yt_playlist';
	/**
	 * Automatic import playlists custom field name
	 * @var string
	 */
	protected $playlist_meta				= '__yti_yt_playlist';
	...
	public function __construct(){
		...
		// plugin filters
		add_filter( 'yti_video_post_content', array( $this, 'format_description' ), 999, 3 );		
	}	
	...
	public function register_post(){
		$labels = array(
			'name' 					=> _x('Videos', 'Videos', 'yti_video'),
	    	'singular_name' 		=> _x('Video', 'Video', 'yti_video'),
	    	'add_new' 				=> _x('Add new', 'Add new video', 'yti_video'),
	    	'add_new_item' 			=> __('Add new video', 'yti_video'),
	    	'edit_item' 			=> __('Edit video', 'yti_video'),
	    	'new_item'				=> __('New video', 'yti_video'),
	    	'all_items' 			=> __('All videos', 'yti_video'),
	    	'view_item' 			=> __('View', 'yti_video'),
	    	'search_items' 			=> __('Search', 'yti_video'),
	    	'not_found' 			=> __('No videos found', 'yti_video'),
	    	'not_found_in_trash' 	=> __('No videos in trash', 'yti_video'), 
	    	'parent_item_colon' 	=> '',
	    	'menu_name' 			=> __('Videos', 'yti_video')
		);
		...
		$args = array(
			...
			'show_in_menu' 			=> true,
			'menu_position' 		=> 5,
			'menu_icon'				=> YTI_URL.'assets/back-end/images/video.png',	
 		...
 		register_post_type( $this->post_type, $args );
  		...
  		$cat_labels = array(
	    	'name' 					=> _x( 'Video categories', 'video', 'yti_video' ),
	    	'singular_name' 		=> _x( 'Video category', 'video', 'yti_video' ),
	    	'search_items' 			=>  __( 'Search video category', 'yti_video' ),
	    	'all_items' 			=> __( 'All video categories', 'yti_video' ),
	    	'parent_item' 			=> __( 'Video category parent', 'yti_video' ),
	    	'parent_item_colon'		=> __( 'Video category parent:', 'yti_video' ),
	    	'edit_item' 			=> __( 'Edit video category', 'yti_video' ), 
	    	'update_item' 			=> __( 'Update video category', 'yti_video' ),
	    	'add_new_item' 			=> __( 'Add new video category', 'yti_video' ),
	    	'new_item_name' 		=> __( 'Video category name', 'yti_video' ),
	    	'menu_name' 			=> __( 'Video categories', 'yti_video' ),
		); 	
	...
	public function updated_messages( $messages ){
		...
		$messages['video'] = array(
	    	...
	    	1 => sprintf( __('Video updated <a href="%s">See video</a>', 'yti_video'), esc_url( get_permalink($post_ID) ) ),
	    	2 => __('Custom field updated.', 'yti_video'),
	    	3 => __('Custom field deleted.', 'yti_video'),
	    	4 => __('Video updated.', 'yti_video'),
	   		/* translators: %s: date and time of the revision */
	    	5 => isset($_GET['revision']) ? sprintf( __('Video restored to version %s', 'yti_video'), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
	    	6 => sprintf( __('Video published. <a href="%s">See video</a>', 'yti_video'), esc_url( get_permalink($post_ID) ) ),
	    	7 => __('Video saved.', 'yti_video'),
	    	8 => sprintf( __('Video saved. <a target="_blank" href="%s">See video</a>', 'yti_video'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
	    	9 => sprintf( __('Video will be published at: <strong>%1$s</strong>. <a target="_blank" href="%2$s">See video</a>', 'yti_video'),
	      	// translators: Publish box date format, see http://php.net/date
	      	date_i18n( __( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink($post_ID) ) ),
	    	10 => sprintf( __('Video draft saved. <a target="_blank" href="%s">See video</a>', 'yti_video'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),

	    	101 => __('Please select a source', 'yti_video'),
	...
	public function import_on_display($null, $object_id, $meta_key, $single){
		...
		$data = yti_set_featured_image( $object_id );
	...
	public function run_import( $raw_feed, $playlist_details ){
		...
		if( $video_ids ){		
			global $wpdb;
			$sql = $wpdb->prepare( 'SELECT post_id, meta_value FROM ' . $wpdb->postmeta . ' WHERE meta_key=%s AND meta_value IN(%s)' , '__yti_video_id', implode("','", $video_ids) );
	...
	protected function import_video( $args = array() ){
		 ...
		 */
		$allow_import = apply_filters('yti_allow_video_import', true, $video, $post_type, $theme_import );
		if( !$allow_import ){
			return false;
		}
		 ...
		 * 
		 */
		$video['description'] = apply_filters('yti_video_description', $video['description'], $options['import_description']);
		...
		// post title
		$video['title'] = apply_filters('yti_video_title', $video['title'], $options['import_title']);
		$post_title 	= $options['import_title'] ? $video['title'] : '';
		
		// action on post insert that allows setting of different meta on post
		do_action('yti_before_post_insert', $video, $theme_import);
		...
		$post_data = array(
			 ...
			 */
			'post_title' 	=> apply_filters('yti_video_post_title', $post_title, $video, $theme_import),
			/**
			 ...
			 * @param bool/array - false if not imported as theme, array if imported as theme and theme is active
			 */
			'post_content' 	=> apply_filters('yti_video_post_content', $post_content, $video, $theme_import),
			/**
			 ...
			 * @param bool/array - false if not imported as theme, array if imported as theme and theme is active
			 */
			'post_excerpt'	=> apply_filters('yti_video_post_excerpt', $post_excerpt, $video, $theme_import),
			'post_type'		=> $post_type,
			'post_status'	=> apply_filters('yti_video_post_status', $status, $video, $theme_import )
		);
		 ...
		 * @param bool/array - false if not imported as theme, array if imported as theme and theme is active
		 */
		$post_date = apply_filters( 'yti_video_post_date', $pd, $video, $theme_import );
		...
		if( import_image_on('post_create') ){				
			...
			yti_set_featured_image( $post_id, $video );
		}	
		...
		if( !is_wp_error($post_id) ){
			...
			// action on post insert that allows setting of different meta on post
			do_action('yti_post_insert', $post_id, $video, $theme_import, $post_type);
			...
			if( $theme_import ){
				...
				$embed_code = '<iframe width="'.$ps['width'].'" height="'.yti_player_height($ps['aspect_ratio'], $ps['width']).'" src="https://www.youtube.com/embed/'.$video['video_id'].'?'.$customize.'" frameborder="0" allowfullscreen></iframe>';
			...
			update_post_meta($post_id, '__yti_video_id', $video['video_id']);
			// set video URL; most likely it will be needed by other plugins
			update_post_meta($post_id, '__yti_video_url', 'https://www.youtube.com/watch?v='.$video['video_id']);
			// store the video data for later use
			update_post_meta($post_id, '__yti_video_data', $video);
			
			// if imported as regular post, flag it as video
			if( !$theme_import && import_as_post() ){	
				// flag post as video post					
				update_post_meta($post_id, '__yti_is_video', true);
			}
	 ...
	 * 
	 * Callback function for filter 'yti_video_post_content' set in class constructor
	 * 
	 * @param $content
	 * @param $video
	 * @param $theme_import
	...
	public function format_description( $content, $video, $theme_import ){
		...
		if( isset( $settings['remove_after_text'] ) ){
			$delimiter = trim( esc_attr( yti_strip_tags( $settings['remove_after_text'] ) ) );
		}
	...
	/**
	 * Callback function for filter 'get_the_categories' set up in function 'YTI_Video_Post_Type->add_to_main_feed'
	 ...
	 * Also verifies regular post type and looks for flag variable '__yti_is_video'
	 */
	public function is_video( $post = false ){
		...
		if( $this->post_type == $post->post_type ){
			$is_video = get_post_meta( $post->ID, '__yti_video_data', true );
			if( $is_video ){
				return true;
			}	
		}
		...
		if( 'post' == $post->post_type ){
			$is_video = get_post_meta( $post->ID, '__yti_is_video', true );
			if( $is_video ){
				return true;
			}
		}
