...
class YTI_Third_Party_Compat{
	...
	public function __construct(){
		...
		if( $this->compatible_theme ){
			...
			if( array_key_exists('autoembed', $this->compatible_theme) ){			
				add_filter( 'yti_video_post_content', array( $this, 'do_autoembeds' ), 30, 3 );
			}
			// if extra meta should be set on post, add the action
			if( array_key_exists('extra_meta', $this->compatible_theme) ){
				add_action( 'yti_post_insert', array( $this, 'add_extra_meta' ), 30, 3 );
			}			
		}		
	}
	
	/**
	 * Action 'yti_post_insert' callback function to set up extra meta fields on posts as required by theme
	 *  
	 * @param int $post_id
	 * @param array $video
	...
	public function add_extra_meta( $post_id, $video, $theme_import ){
		...
		$player_settings = yti_get_player_settings();
		...
		foreach( $extra_meta as $meta_key => $data ){
			switch( $data['type'] ){
				...
				case 'video_data':
					...
					switch( $data['value'] ){
						case 'human_duration':
							$value = yti_human_time( $video['duration'] );
						break;
						case 'rating':
						case 'rating_count':
						case 'views':
	...
	/**
	 * Filter 'yti_video_post_content' callback to put video link in post content for autoembeds 
	 *
	...
	private function check_compatibility(){
		...
		$themes = apply_filters( 'yti_youtube_theme_support', $this->wp_themes() );
	...
	private function wp_themes(){
		...
		$themes = array(
			...
			'truemag' => array(
				...
				'post_format' 	=> 'video',
				'theme_name' 	=> 'TrueMag',
				'url'			=> 'http://www.themeforest.net/item/true-mag-wordpress-theme-for-video-and-magazine/6755267/',
				'extra_meta' 	=> array(
					'time_video' => array(				
						'type' 	=> 'video_data',
			...
			'goodwork' => array(
				...
				'post_format'	=> 'video',
				'theme_name' 	=> 'Goodwork',
				'url'			=> 'http://themeforest.net/item/goodwork-modern-responsive-multipurpose-wordpress-theme/4574698/'	
			),
			...
			'simplemag' => array(
				...
				'url' => 'http://themeforest.net/item/simplemag-magazine-theme-for-creative-stuff/4923427/'
			),			
			...
			'sahifa' => array(
				...
				'post_format' 	=> 'video',
				'theme_name' 	=> 'Sahifa',
				'url'			=> 'http://themeforest.net/item/sahifa-responsive-wordpress-newsmagazineblog/2819356/',
			...
			'wave' => array(
				...
				'post_format'	=> 'video',
				'theme_name' 	=> 'Wave',
				'url'			=> 'http://themeforest.net/item/wave-video-theme-for-wordpress/45855/',
				'extra_meta'	=> array(
			...
			'detube' => array(
				...
				'url'			=> 'http://themeforest.net/item/detube-professional-video-wordpress-theme/2664497/'
			),
			...
			'beetube' => array(
				...
				'post_format'	=> 'video',
				'theme_name' 	=> 'Beetube',
				'url'			=> 'http://themeforest.net/item/beetube-video-wordpress-theme/7055188/',
				'extra_meta' => array(
					'views' => array(
						'type' => 'video_data',
						'value' => 'views'
 ...
 * 
 * Callback function on action 'yti_post_insert'
 * 
 * @param int $post_id
 * @param array $video
 * @param array $theme_import
...
function yoast_video_seo_compatibility(  $post_id, $video, $theme_import, $post_type ){
	...
	$meta_title = esc_attr( yti_strip_tags(  $video['title'] ) );
...
add_action( 'yti_post_insert', 'yoast_video_seo_compatibility', 10, 4 );

/**
 * Callback function for filter "wpseo_build_sitemap_post_type" in Yoast WP SEO
 * @param string $type - sitemap type
 */
function yti_filter_yoast_sitemap_type( $type ){
...
add_filter('wpseo_build_sitemap_post_type', 'yti_filter_yoast_sitemap_type', 10, 1);
 ...
 */
/*
function yti_ultimate_seo_fields( $post_id, $video, $theme_import ){
...
add_action('yti_post_insert', 'yti_ultimate_seo_fields', 10, 3);
*/